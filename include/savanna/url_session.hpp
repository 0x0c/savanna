...
namespace savanna
{
	...
	private:
		...
		static http::response<Body> send_request(Stream &stream, savanna::url url, http::verb method, boost::optional<std::map<std::string, std::string>> params, bool follow_location, int version, std::map<std::string, std::string> header_fields, std::string body)

		{
			...
			for (auto h = header_fields.begin(); h != header_fields.end(); ++h) {
			...
			if (response.result_int() >= 300 && response.result_int() < 400 && follow_location) {
				...
				response = send_request<Stream, Body>(stream, savanna::url(location), method, params, follow_location, version, header_fields, body);
				// response = send_request<Body>(savanna::url(location));
			}
		...
		static http::response<Body> send_request(savanna::request_t<Endpoint> request)
		{
			...
			if (scheme == url_scheme::https) {
				...
				auto response = send_request<beast::ssl_stream<beast::tcp_stream>, Body>(stream, url, request.endpoint.method(), request.endpoint.params(), request.follow_location, request.version, request.header_fields, request.body);
				...
				auto response = send_request<beast::tcp_stream, Body>(stream, url, request.endpoint.method(), request.endpoint.params(), request.follow_location, request.version, request.header_fields, request.body);
	...
	public:
		...
		savanna::result_t<http::response<Body>> send(savanna::request_t<Endpoint> request)
		{
			...
			try {
				http::response<Body> response = send_request<Body, Endpoint>(request);
